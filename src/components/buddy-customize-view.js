...
class BuddyCustomizeView extends LitElement {
    ...
    _onSaveProfile() {
        ...
        try {
            ...
            this.dispatchEvent(new CustomEvent('create-profile', { 
            ...
            <div class="main-view-container">
                ...
                <div class="option-group ${selectedProfileData ? 'active-profile' : ''}">
                    ...
                    <div class="profile-selector ${selectedProfileData ? 'selected' : ''}">
                        ...
                        ${selectedProfileData ? html`
                            <div class="selected-profile-indicator">✓</div>
                        ` : ''}
                    </div>
                    ...
                    ${selectedProfileData?.isCustom ? html`
                ...
                ${this.isCreatingProfile ? html`
                    <div class="option-group create-profile-form">
                        <h3>✨ Create New Profile</h3>
                        
                        ${this.validationErrors.general ? html`
                            <div class="general-error">${this.validationErrors.general}</div>
                        ` : ''}
                        ...
                        <input 
                            ...
                            class=${this.validationErrors.name ? 'input-error' : ''}
                            maxlength="50"
                            @input=${(e) => {
                        ...
                        ` : ''}
                        <div class="character-counter ${(this.newProfileName?.length || 0) > 40 ? 'warning' : ''} ${(this.newProfileName?.length || 0) > 50 ? 'error' : ''}">
                        ...
                        <input 
                            ...
                            class=${this.validationErrors.description ? 'input-error' : ''}
                            maxlength="200"
                            @input=${(e) => {
                        ...
                        ${this.validationErrors.description ? html`
                            <div class="error-message">${this.validationErrors.description}</div>
                        ` : ''}
                        <div class="character-counter ${(this.newProfileDescription?.length || 0) > 160 ? 'warning' : ''} ${(this.newProfileDescription?.length || 0) > 200 ? 'error' : ''}">
                        ...
                        <textarea
                            ...
                            class=${this.validationErrors.prompt ? 'input-error' : ''}
                            rows="6"
                            maxlength="2000"
                            @input=${(e) => {
                        ...
                        ></textarea>
                        ${this.validationErrors.prompt ? html`
                            <div class="error-message">${this.validationErrors.prompt}</div>
                        ` : ''}
                        <div class="character-counter ${(this.newProfilePrompt?.length || 0) > 1600 ? 'warning' : ''} ${(this.newProfilePrompt?.length || 0) > 2000 ? 'error' : ''}">
                            ${this.newProfilePrompt?.length || 0}/2000 characters
                        </div>
                        
                        <div class="button-group">
                            <button 
                                class="button ${this.isLoading ? 'loading' : ''}" 
                            ...
                            <button 
                                ...
                                @click=${this._onCancelCreateProfile}
                                ?disabled=${this.isLoading}
                            >
                                ❌ Cancel
                            </button>
                        ...
                        <div class="description">
                            <strong>💡 Tips:</strong> Create specialized profiles for different tasks like code review, creative writing, technical documentation, or tutoring. The more specific your system prompt, the better the AI will perform in that role.
                        </div>
                    </div>
                ` : ''}
                ...
                <div class="option-group">
                    <label class="option-label">Additional Custom Instructions</label>
                    <textarea
                        placeholder="Add extra instructions for the current session..."
                    ...
                    ></textarea>
                    <div class="description">
                        These instructions will be added to the ${selectedProfileData?.name || 'selected profile'} for this session only.
                    </div>
